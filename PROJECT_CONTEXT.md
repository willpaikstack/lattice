...
- The public site leads with a manufacturing-proof landing page for domestic machine shops managing overflow and cyclical demand. The hero describes Lattice as qualified global CNC machining and fabrication capacity that protects scheduled commitments, lead times, and customer responsiveness; its quality section explains that requested inspection reports and material documentation are uploaded to Lattice for customer review before shipment. The header's primary action is now `Request an account` to the waiting-list flow, rather than a public quote entry. It retains its capabilities, materials, quality, and workflow controls as disabled visual placeholders so they cannot bypass the current access policy. Guest simple quote requests remain a separately implemented workflow under `/simple-quote`, with `requestOrigin: GUEST_SIMPLE_QUOTE`, admin-quote visibility, acknowledgement/quote-ready email handling, tokenized quote review, and card-only Stripe payment.
