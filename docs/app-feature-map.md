...
| Dashboard command center | `/dashboard` | Leads with four operational KPI cards, followed by the workflow-based Action Center and quote/order activity. | `listBuyerQuotes()`, `listBuyerOrders()`, `buildCustomerDashboardSummary()`, `buildCustomerActionWorkflows()`, exact-email customer ownership filter. | Operational | Notification history lives only in `/notifications`; values are derived from existing RFQ/order records and filtered to the signed-in customer's exact requester email unless the session is admin. |
...
| Notification center | `/notifications` | Shows the full chronological customer activity history, including action-related events, lower-level RFQ history, operator updates, and customer-facing request-info/no-quote notes. | `buildCustomerActivityFeed()` plus open workflow count from `buildCustomerActionWorkflows()`. | Operational | This is the single in-app location for notification history. Reading a notification does not resolve an action workflow. V1 has no persistent read/unread state. Working spec: `docs/customer-inbox-notification-spec.md`. |
