...
- The public site now leads with a manufacturing-proof landing page for overflow CNC and fabrication work. Its primary CTA opens the account-free `/simple-quote` lane, while supporting navigation exposes capabilities, materials, quality documentation, workflow proof, and login. Guest simple quote requests persist into the normal `Request` workflow with `requestOrigin: GUEST_SIMPLE_QUOTE`, appear in `/admin/quotes`, receive acknowledgement and quote-ready emails through the Resend/local-outbox path, and use tokenized magic links under `/simple-quote/[requestId]` for quote review and card-only Stripe payment without account access.
