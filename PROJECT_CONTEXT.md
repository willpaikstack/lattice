...
15. Buyer-facing quote lifecycle tags are `Draft`, `Quote Requested`, `Quote Received`, `In Production`, `Shipping`, `Delivered`, and `Archived`. Current triggers map submitted, needs-info, and supplier-ready RFQs to `Quote Requested`; quoted RFQs to `Quote Received`; purchased work before shipment to `In Production`; shipped work to `Shipping`; and archived or closed records to `Archived`. `Delivered` is reserved until the order model stores a durable delivery-confirmation event or status.
