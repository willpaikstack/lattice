...
| Feature | Routes | What It Does | Data Source | Status | Notes |
| --- | --- | --- | --- | --- | --- |
| Customer shell | Customer routes under `/dashboard`, `/requests`, `/quotes`, `/orders`, `/roadmap`, `/materials`, `/capabilities`, `/equipment`, `/notifications`, `/account/settings` | Provides the buyer-facing app frame, sidebar, role-aware admin bridge, customer navigation, and a global notification-bell link on desktop/mobile. | Session role and app-shell components. | Operational | Admin sessions can operate customer routes for development/support simulation. The bell intentionally has no unread badge until durable read state exists. |
| Dashboard command center | `/dashboard` | Leads with a workflow-based Action Center, followed by operational KPI cards, Recent Updates, and quote/order activity. | `listBuyerQuotes()`, `listBuyerOrders()`, `buildCustomerDashboardSummary()`, `buildCustomerActionWorkflows()`, exact-email customer ownership filter. | Operational | No separate dashboard table; values are derived from existing RFQ/order records and filtered to the signed-in customer's exact requester email unless the session is admin. |
| Customer Action Center | `/dashboard#action-center` | Groups unresolved supplier clarification, quote review/expiration, overdue order milestone, and customer-document states into prioritized workflows with owner, due context, progress, checklist steps, and a continuation action. | `buildCustomerActionWorkflows()` in `src/lib/customer-action-center.ts`, using existing RFQ/quote/order state. | Operational | V1 checklist progress is state-derived rather than user-persisted. Quote review and expiration collapse into one workflow to avoid duplicates. Explicit customer replies and durable step completion remain future work. |
| Dashboard Recent Updates | `/dashboard` | Shows the newest high-signal informational and action-related events without mixing them into the workflow queue. Lower-level self-generated audit events such as draft creation remain in full notification history only. | `buildCustomerActivityFeed()` in `src/lib/customer-notifications.ts`, filtered by `buildCustomerDashboardSummary()`. | Operational | This is a preview, not an unread Inbox. Working spec: `docs/customer-inbox-notification-spec.md`. |
| Notification center | `/notifications` | Shows the full chronological customer activity history, including action-related events, lower-level RFQ history, operator updates, and customer-facing request-info/no-quote notes. | Same derived feed source as dashboard Recent Updates; open workflow count comes from `buildCustomerActionWorkflows()`. | Operational | Reading a notification does not resolve an action workflow. V1 has no persistent read/unread state. Working spec: `docs/customer-inbox-notification-spec.md`. |
| New RFQ request | `/requests/new` | Leads with CAD upload, a visible Upload/Configure/Review/Submit progression, autosave reassurance, and a continuation list limited to genuine unsubmitted drafts; uploaded files then unlock the existing multi-line-item material/process configuration, Hubs-aligned material metadata, ISO 2768-1 tolerance reference popup, surface finish secondary choices, inspection/documentation choices, drawing-required validation, ship-to/requester snapshots, draft recovery, and reorder flow. | Request form, CNC material library, RFQ option metadata, local draft upload storage, request API, Prisma/local fallback, exact-email ownership filter for reorder/draft prefill. | Operational | Submitted RFQs no longer appear under draft continuation. Material dropdown options show UNS numbers and compact composition formulas where applicable. Tolerances include a compact Hubs-style ISO 2768-1 standards modal. Configured line items can be deleted from the line-item header after a confirmation modal. Surface finishes expose only the relevant cosmetic requirement, color, or RAL/Pantone fields for each selected finish. Production file storage still needs R2/S3 replacement for local `.data/uploads`. |
| Buyer quotes list | `/quotes` | Shows active RFQs and quote-received records, excluding purchased quotes, sorted by most recently edited and paginated three rows at a time per section. | Buyer quote repository, lifecycle helper, exact-email ownership filter. | Operational | Purchased records live in Orders. Customer sessions see only matching requester-email records; admins see all for support. |
| Buyer quote detail | `/quotes/[requestId]` | Shows a status/next-step summary, responsive part and pricing rows, quote activity, CAD thumbnails, quote PDF, purchase actions, and customer-facing operator notes for missing-info or no-quote outcomes. | Request record, customer quote versions, uploaded files, APS preview URNs, exact-email ownership policy. | Operational | Part details stack before the layout becomes too narrow and do not require horizontal page scrolling. Purchased quote detail redirects to order detail. Unauthorized customer direct URLs return not found. `NEEDS_INFO` reads as a clarification request; `CLOSED` with an operator note reads as `No quote` / unable to quote. Buyer quote modification is intentionally deferred for now. |
...
| Customer dashboard summary | `src/lib/customer-dashboard.ts` | Builds dashboard KPIs, grouped action workflows, Recent Updates, and quote/order activity rows for quote-received and order-placed events. | Buyer quotes and buyer orders. | Operational | Keep dashboard and notifications aligned through shared feed helpers. The old mixed Inbox and right-side order/contact card have been removed from `/dashboard`. |
