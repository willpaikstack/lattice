...
- The public site now leads with a manufacturing-proof landing page for overflow CNC and fabrication work. Its primary CTA opens the account-free `/simple-quote` lane; the header retains its capabilities, materials, quality, and workflow controls as disabled visual placeholders so they cannot bypass the current access policy. Guest simple quote requests persist into the normal `Request` workflow with `requestOrigin: GUEST_SIMPLE_QUOTE`, appear in `/admin/quotes`, receive acknowledgement and quote-ready emails through the Resend/local-outbox path, and use tokenized magic links under `/simple-quote/[requestId]` for quote review and card-only Stripe payment without account access.
