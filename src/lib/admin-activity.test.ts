...
describe("admin activity summary", () => {
  it("summarizes active quote requests for the administrator control surface", () => {
    const submitted = makeRequest({ id: "req_submitted", dueDate: "2026-05-20", updatedAt: "2026-05-20T10:00:00.000Z" });
    ...
    expect(summary.metrics.activeQuoteRequests).toBe(4);
    expect(summary.metrics.needsAdminAction).toBe(2);
    expect(summary.metrics.blockedRequests).toBe(1);
    expect(summary.metrics.supplierReady).toBe(1);
    expect(summary.metrics.unassignedRequests).toBe(1);
    expect(summary.metrics.overdueRequests).toBe(1);
    expect(summary.metrics.buyerDecisionPending).toBe(1);
    expect(summary.metrics.quotedValueCents).toBe(125000);
    ...
    expect(summary.criticalRequests.map((request) => request.requestId)).toEqual(["req_submitted", "req_needs_info", "req_ready", "req_quoted"]);
