...
- Customer-facing RFQ, quote, order, checkout, invoice, and submitted-file access is scoped below the role guards by durable customer-company membership. `User` records carry a `WorkspaceRole` (`LATTICE_ADMIN`, `CUSTOMER_ADMIN`, or `CUSTOMER_MEMBER`) and optional `companyId`; every provisioned user at a customer company can access that company's work, while Lattice admins retain global support access. New customer RFQs are connected to the signed-in company. After applying the schema, use `npm run onboard:lattice-admin -- --name "..." --email "..."` for the sole Lattice Admin and `npm run onboard:customer -- --company "..." --name "..." --email "..."` to provision the first customer admin. Suppliers remain operator-managed without platform access in this phase.
...
- Resend and Cloudflare R2/S3 production storage are still pending. Workspace routes are protected by a signed HTTP-only session cookie; the login page presents work email and password together, with Google Workspace SSO through Google OAuth/OIDC available when configured. Durable Lattice/customer role and company membership records now hydrate server sessions and the Google callback; password recovery, MFA/passkeys, SSO enforcement, session/device controls, invitation delivery, and audit events remain future hardening.
