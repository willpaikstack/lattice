...
- The public site leads with a manufacturing-proof landing page for domestic machine shops managing overflow and cyclical demand. The hero describes Lattice as qualified global CNC machining and fabrication capacity that protects scheduled commitments, lead times, and customer responsiveness; its quality section explains that requested inspection reports and material documentation are uploaded to Lattice for customer review before shipment. The header's primary action is `Request an account` to the invite-only waiting-list flow, whose fit-focused request form explains qualified capacity, managed execution, and pre-shipment quality evidence before collecting company and work details. Capabilities, materials, and quality remain disabled visual placeholders, while `How it works` links to the account-safe public `/how-it-works` one-pager describing the approved-account workflow and pre-shipment document review. Guest simple quote requests remain a separately implemented workflow under `/simple-quote`, with `requestOrigin: GUEST_SIMPLE_QUOTE`, admin-quote visibility, acknowledgement/quote-ready email handling, tokenized quote review, and card-only Stripe payment.
...
- Resend and Cloudflare R2/S3 production storage are still pending. Workspace routes are protected by a signed HTTP-only session cookie; the login page presents work email and password together, with Google Workspace SSO through Google OAuth/OIDC available when configured. Login failures preserve the work email and intended destination, while the final durable multi-user identity, MFA/passkey, and organization-policy model remains future hardening.
...
- `/simple-quote`, `/simple-quote/thanks`, `/simple-quote/[requestId]`, `/simple-quote/[requestId]/quote.pdf`, and `/simple-quote/[requestId]/success` - public account-free simple quote intake, submission confirmation, tokenized quote review/PDF download, and credit-card payment success pages. Guests can only access their specific quote by token and cannot enter the account workspace.
...
- `src/lib/auth-crypto.ts`, `src/lib/session.ts`, `src/lib/google-sso.ts`, `src/lib/route-authorization.ts`, `src/app/login/actions.ts`, `src/app/api/auth/google/`, and `src/proxy.ts` - local credential authentication, Google Workspace SSO, signed session cookie helpers, role guards for route handlers/server actions, login action, and optimistic route protection.
- `src/app/simple-quote/`, `src/components/simple-quote-form.tsx`, `src/components/guest-quote-checkout.tsx`, `src/lib/guest-quote-access.ts`, and `src/lib/guest-quote-email.ts` - public guest simple quote intake, magic-link validation, quote-ready/acknowledgement emails, and tokenized card-only checkout.
